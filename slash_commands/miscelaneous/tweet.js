...
async function gatherTweets(client, username, interaction) {
    ...
    try {
        ...
        return interaction.editReply('No ha sido posble encontrar el usuario.');
    }
    ...
    try {
        ...
        return interaction.editReply('El usuario no cuenta con tweets o la cuenta es privada.');
    }
    ...
    try {
        ...
        if (webhooks.first() === undefined) {
            ...
            if (hq_video_url != 'none') {
                await sendWebhook(channel, hq_video_url);
            }
            await webhook.send({
                // content: content_tweet,
                username: name_user,
...
async function sendWebhook(channel, text) {
    try {
		...
		const webhook = webhooks.first();
        await webhook.send({
            content: '`Archivo multimedia`' + hider + text,
            username: name_user,
            avatarURL: url_avatar,
        });
    } catch (error) {
